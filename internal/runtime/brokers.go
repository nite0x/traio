...
func (g gatewayAdapter) LoginURL() string                       { return g.m.LoginURL() }
func (g gatewayAdapter) StartGateway(ctx context.Context) error { return g.m.StartGateway(ctx) }
func (g gatewayAdapter) StopGateway(keepSession bool) error     { return g.m.StopGateway(keepSession) }
func (g gatewayAdapter) Reconnect() error                       { return g.m.Reconnect() }
func (g gatewayAdapter) Upgrade(ctx context.Context) error      { return g.m.Upgrade(ctx) }
...
func (b Brokers) ApplyConfig(updated config.Config) {
	...
	go func() { _ = b.gateway.Reconfigure(updated.IBKR) }()
}
